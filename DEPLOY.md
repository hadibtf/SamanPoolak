...
**platform.samanpoolak.ir** subdomain (this app's front-end), the
...
- Each deploy command asks for a mode:
  ...
  - `manual` creates zip packages in `deploy-manual/` and writes
    `UPLOAD-INSTRUCTIONS.md` beside them.
  You can skip the prompt with `--mode=automatic` or `--mode=manual`, for example:
  `node scripts/deploy.mjs all --mode=manual`.
...
3. **Subdomains** — create `platform.samanpoolak.ir` and `api.samanpoolak.ir`
   in cPanel → *Domains*; note their doc roots (`platform/`, `api/`).
...
6. **config.php** — in `api/`, copy `config.sample.php` → `config.php` and fill
   ...
   `https://platform.samanpoolak.ir` and `http://localhost:3000`), and a
...
10. **Front-end** — `npm run deploy:web` to upload `build/` to `platform/`. The
   deploy **skips `.htaccess`**, so seed the doc root once with the SPA rewrite
